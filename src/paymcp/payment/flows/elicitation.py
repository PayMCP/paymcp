...
def make_paid_wrapper(func, mcp, provider, price_info):
    ...
    async def _confirm_tool(payment_id: str):
        ...
        logger.debug(f"[elicitation_confirm_tool] Looking up session with provider={provider_name} payment_id={payment_id}")
        ...
        logger.debug(f"[elicitation_confirm_tool] Calling {func.__name__} with stored args")
