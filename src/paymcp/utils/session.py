...
def extract_session_id(ctx):
    ...
    if hasattr(ctx, 'request'):
        ...
        if hasattr(request, 'headers'):
            ...
            if isinstance(headers, dict):
                for key, value in headers.items():
                    if key.lower() == 'mcp-session-id':
                        logger.debug(f"Found MCP session ID in request.headers: {value}")
                        return value
            elif hasattr(headers, 'get'):
                session_id = headers.get('Mcp-Session-Id') or headers.get('mcp-session-id')
                if session_id:
                    logger.debug(f"Found MCP session ID via request.headers.get: {session_id}")
                    return session_id
    ...
    if (hasattr(ctx, 'headers') or
        (hasattr(ctx, 'request') and hasattr(ctx.request, 'headers')) or
        (hasattr(ctx, '_meta') and hasattr(ctx._meta, 'headers'))):
