...
class TestExtractSessionId:
    ...
    def test_headers_object_with_get_method(self):
        ...
        headers_mock.get = Mock(
            side_effect=lambda k: "session-abc" if k == "Mcp-Session-Id" else None
        )
    ...
    def test_headers_object_get_lowercase(self):
        ...
        headers_mock = Mock()
        headers_mock.get = Mock(
            side_effect=lambda k: "session-def" if k == "mcp-session-id" else None
        )
        ctx.headers = headers_mock
        result = extract_session_id(ctx)
    ...
    def test_request_headers_object_with_get(self):
        ...
        headers_mock = Mock()
        headers_mock.get = Mock(
            side_effect=lambda k: "session-req-obj" if k == "Mcp-Session-Id" else None
        )
    ...
    def test_stdio_transport_no_warning(self):
        ...
        mock_logger.debug.assert_called_with(
            "No MCP session ID found (STDIO transport)"
        )
